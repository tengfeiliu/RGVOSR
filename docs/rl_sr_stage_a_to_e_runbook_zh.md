...
4. 对 `G_SFT` 和最终 `G_RL` 自动在 `RealLQ250`、`RealLR200` 进行 1–`iterations` 轮推理与指标评估。
...
再把原始 JSONL 直接传给推理器。配置中的 `expected_count: 250/200` 会在开始推理前
验证两套数据是否完整。
每次启动会创建唯一目录，名称包含 F0 step、FLUX 配置、prompt、condition、轮数、
候选数、训练步数和时间，例如
`rlsr_f0-036000_flux2-klein_flux2-image-concat_iqa-suggestion_ccondition8_s512_r4_k4_sft1000_nft250_eval-reallq250-reallr200_260912-101530`。

评估 JSONL 每条记录必须有可访问的 `lq_path`，以及与当前 `iqa_suggestion + caption`
设置匹配的 `unipercept_raw.profile`。记录还应以 `dataset` 或 `dataset_name` 标记为
`RealLQ250`、`RealLR200`，供脚本精确分组。当前评估是 NR-IQA，因此不需要 HR、
...
`round_01/RealLQ250/`、`round_01/RealLR200/`，其下的 `metrics/` 保存逐图和汇总
NR-IQA。脚本还会生成
