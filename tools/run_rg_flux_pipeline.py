...
DEFAULT_METRICS = ["clipiqa", "clipiqa+", "nima", "niqe", "liqe", "musiq", "maniqa-pipal"]
PROMPT_VARIANTS = (
    "fixed",
    "suggestion",
    "iqa",
...
def apply_prompt_variant(config, prompt_variant):
    ...
    if isinstance(prompt_schedule, dict) and cfg_bool(config, "condition.prompt_schedule.enabled", False):
        ...
        if prompt_variant == "fixed":
            prompt_schedule["after_include_caption"] = False
    return config
...
def build_inference_command(
    ...
    _append_optional(cmd, "--inference_schedule", getattr(args, "inference_schedule", None))
    _append_optional(cmd, "--inference_init_mode", getattr(args, "inference_init_mode", None))
    _append_optional(cmd, "--inference_sigma_start", getattr(args, "inference_sigma_start", None))
...
def build_arg_parser():
    ...
    parser.add_argument("--inference_schedule", choices=["linear", "empirical_shift"], default=None)
    parser.add_argument("--inference_init_mode", choices=["pure_noise", "lr_warm_start"], default=None)
